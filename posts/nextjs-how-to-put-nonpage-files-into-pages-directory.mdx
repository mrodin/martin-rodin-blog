...
One thing that always bugged me about Next.js was the fact that I was unable to collocate my code with the particular route in the `pages{:shell}` directory. It was a long standing rule that you shouldn’t put any non-page files in your `pages{:shell}` folder because Next.js automatically treats every file there as a new route.
...
To be able to put non-page files into the `pages{:shell}` directory, you need to tell Next.js how to differentiate between pages and the rest of the files. You can do this in the `next.config.js{:shell}` file. If this file doesn’t exist in your project yet, simply create it in the root folder (next to `package.json{:shell}` file) using `touch{:shell}` command:

```shell
touch next.config.js
```

Then we would like to specify that only files with `.page.tsx|ts|jsx|js{:shell}` extensions (for example `products.page.tsx{:shell}`) should be considered page files. Add this configuration into your `next.config.js{:shell}` file:
...
We moved all our module files into the pages folder and defined `index.page.ts{:shell}` as our page file.
