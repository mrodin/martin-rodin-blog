...
To do so, we need to export the `getServerSideProps{:js}` function from the respective page (index.js page in my example).
...
The `getTopHeadlines{:js}` function is a simple wrapper function around JS fetch. It looks like this:
...
Since the news sources rarely change, we are going to make this page completely static and generate it on build time. The way we do this is very similar to the previous example, but we are going to export `getStaticProps{:js}` function from the page instead of `getServerSideProps{:js}`.
...
`localhost:3000/source/techcrunch{:shell}`

In Next.js we can create dynamic route using brackets, e.g.:

`pages/source/[id].js{:shell}`

Now when we want to pre-render a dynamic route page using `getStaticProps{:js}`, we need to provide a complete list of static paths (all valid news sources) that Next.js should render during build time. We can do this with the `getStaticPaths{:js}` function.
...
In `getStaticPaths{:js}` we will firstly fetch all the news sources from an API to get the complete list of all of them and store them into a `sources{:js}` constant. Then we create a `paths{:js}` constant which will contain an array of objects with `params{:js}` key. In our case the `paths{:js}` array is going to look like this:
...
This way Next.js will pre-render `source/engadget{:shell}`, `source/hacker-news{:shell}`, etc. using the dynamic page `pages/source/[id].js{:shell}`. Note that the value for each `params{:js}` must match the parameter in the page name. In our case, we have a page called `[id].js{:shell}`, so the key inside the `params{:js}` object must also be `id{:js}`.

Now that we have our `getStaticPaths{:js}` function, we will also need to export a `getStaticProps{:js}` function to tell Next.js that our source detail page should be statically pre-rendered during build time.
...
Notice that `getStaticProps{:js}` function takes parameter `context{:js}`, which we destructure to obtain a `params{:js}` object containing the route parameters for the page. In our case, the `params{:js}` will look like e.g. `{ id: engadget }{:js}`. So we can use the `params.id{:js}` to get the list of headlines from the corresponding news source.
...
Incremental static regeneration is a feature of Next.js that allows you to “re-generate” (meaning update) static pages as users visit your website. You can enable this feature in code using the `revalidate{:js}` parameter in the return of your `getStaticProps{:js}` function.
